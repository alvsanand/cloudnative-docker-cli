...
{
echo 'Installer for cloudnative-docker-cli and configure a profile in Windows Terminal.
...
if ($args[0] -match "--restore") {
    ...
    echo "Deleting Docker container[$DOCKER_ID]"
    ...
    if ($DOCKER_ID) {
        ...
        echo "Deleted Docker container[$DOCKER_ID]"
    }
}
...
if (!$DOCKER_ID) {
    ...
    docker run --restart=always -d `
        -v "/$($env:USERPROFILE.replace('\','/').replace(':',''))/:/data/home" `
        -v "/$($env:USERPROFILE.replace('\','/').replace(':',''))/.aws:/home/cloudnative-docker-cli/.aws" `
    ...
    if (!$DOCKER_ID) {
        echo "Error launching Docker container"
        Exit 1
    }
    
    echo "Launched Docker container[$DOCKER_ID]"
    ...
    if (Select-String -Path "$WT_CONFIG_FILE" -Pattern "CLOUDNATIVE-DOCKER-CLI") {
        ...
        echo "Updated profile[CLOUDNATIVE-DOCKER-CLI] Windows Terminal configuration[$WT_CONFIG_FILE]"
    } else {
...
"@
        ...
        echo "Created profile[CLOUDNATIVE-DOCKER-CLI] Windows Terminal configuration[$WT_CONFIG_FILE]"
    }
}